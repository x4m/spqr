...
operator_option
    ...
    | (LEFTARG | RIGHTARG) EQUAL x_type=data_type
...
sequence_body
    : AS x_type=(SMALLINT | INTEGER | BIGINT)
...
constr_body
    ...
    | (FOREIGN KEY names_in_parens)? REFERENCES schema_qualified_name ref=names_in_parens?
        (MATCH (FULL | PARTIAL | SIMPLE) | ON (DELETE | UPDATE) x_action)*
...
diagnostic_option
    : x_var (COLON_EQUAL | EQUAL) identifier
    ;
...
assign_stmt
    : x_var (COLON_EQUAL | EQUAL) (select_stmt_no_parens | perform_stmt)
    ;
...
cursor_statement
    : OPEN x_var (NO? SCROLL)? FOR plpgsql_query
    | OPEN x_var (LEFT_PAREN option (COMMA option)* RIGHT_PAREN)?
    | FETCH fetch_move_direction? (FROM | IN)? x_var
    | MOVE fetch_move_direction? (FROM | IN)? x_var
    | CLOSE x_var
    ;
